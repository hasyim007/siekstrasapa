        ...
        const DEFAULT_SETTINGS = {
            ...
            adminPassword: 'admin',
            geminiApiKey: '' // dipakai untuk fitur KPI & Kesimpulan berbasis AI di Dashboard
        };
        ...
        const uiManager = {
            ...
            renderBerandaCards() {
                ...
                document.getElementById('counter-ekskul-beranda').innerText = `${state.ekskul.length} Kegiatan`;
                ...
                container.innerHTML = state.ekskul.map(item => {
                    ...
                    <div class="glass-card p-5 md:p-6 rounded-2xl h-full flex flex-col justify-between group">
                        <div>
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <span class="px-2.5 py-1 text-[10px] font-bold bg-white text-slate-500 rounded-lg uppercase border border-slate-100">Buka Pendaftaran</span>
                            </div>
            ...
            populateEkskulCheckboxes() {
                ...
                container.innerHTML = state.ekskul.map(ek => `
                    <label class="flex items-start gap-3 p-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 cursor-pointer transition-colors has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50/50">
                        <div class="flex-shrink-0 pt-0.5">
                            <input type="checkbox" name="sv-ekskul" value="${ek.nama}" class="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500">
                        </div>
                        <div class="flex-1">
            ...
            openKelasModal(id = null) {
                ...
                if(id) {
                    ...
                    if(item) { document.getElementById('kelas-id').value = item.id; document.getElementById('kelas-nama').value = item.nama; }
                } else {
                    document.getElementById('modal-kelas-title').innerText = "Tambah Kelas Baru";
                    document.getElementById('kelas-id').value = ''; document.getElementById('kelas-nama').value = '';
                }
            ...
            openEkskulModal(id = null) {
                ...
                if(id) {
                    ...
                    document.getElementById('ekskul-id').value = ''; document.getElementById('ekskul-nama').value = ''; document.getElementById('ekskul-deskripsi').value = '';
                    masterDataManager.resetDynamicFields();
        ...
        const settingsManager = {
            ...
            renderForm() {
                ...
                if (geminiEl) geminiEl.value = s.geminiApiKey || '';
            },
            ...
            save() {
                ...
                const geminiApiKey = geminiEl ? geminiEl.value.trim() : (state.settings.geminiApiKey || '');
        ...
        const classManager = {
                    ...
                    tbody.innerHTML += `
                        <tr class="hover:bg-indigo-50/40 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-[10px] text-slate-500 mt-1">${studentCount} Siswa terdaftar</div>
                            </td>
            ...
            saveKelas(e) {
                ...
                if(id) {
                    ...
                    state.kelas.push({ id: system.generateId('kls'), nama });
        ...
        const masterDataManager = {
            renderTable() {
                ...
                if(state.ekskul.length === 0) { tbody.innerHTML = `<tr><td colspan="4" class="px-6 py-8 text-center text-sm text-slate-500">Belum ada daftar ekstrakurikuler.</td></tr>`; return; }
            ...
            saveEkskul(e) {
                ...
                if(id) { 
                    ...
                    if(idx !== -1) state.ekskul[idx] = {id, nama, pembimbing, grup, deskripsi}; 
                    system.showToast('Data ekstrakurikuler diubah.');
                }
                else {
                    state.ekskul.push({id: system.generateId('ek'), nama, pembimbing, grup, deskripsi});
                    system.showToast('Ekstrakurikuler berhasil ditambahkan.');
                }
                system.saveData('ekskul'); uiManager.closeEkskulModal(); this.renderTable(); uiManager.renderBerandaCards(); uiManager.populateEkskulDropdowns(); uiManager.populateEkskulCheckboxes();
            },
            deleteEkskul(id, nama) {
                uiManager.confirm(`Cabut ekstrakurikuler "${nama}" dari daftar?`, () => {
                    state.ekskul = state.ekskul.filter(e => e.id !== id);
                    system.saveData('ekskul'); this.renderTable(); uiManager.renderBerandaCards(); uiManager.populateEkskulDropdowns(); uiManager.populateEkskulCheckboxes();
        ...
        const saranManager = {
            ...
            submitPublic(e) {
                ...
                if (!nama || !noWa || !pesan) {
                    return system.showToast('Nama, No. WA, dan isi saran/kritik wajib diisi.', 'error');
                }
            ...
            renderTable() {
                ...
                container.innerHTML = filtered.map(s => `
                    <div class="glass-card p-5 rounded-2xl flex flex-col gap-3">
                        ...
                        <p class="text-xs text-slate-600 bg-slate-50 border border-slate-100 rounded-xl p-3 leading-relaxed">"${s.pesan}"</p>
                ...
                this.activePhotos.forEach((photo, idx) => {
                    ...
                    slidesHTML += `
                        <div class="absolute inset-0 transition-opacity duration-1000 ease-in-out ${idx === 0 ? 'opacity-100 z-10' : 'opacity-0 z-0'}" id="slide-${idx}">
                            <img src="${imgUrl}" alt="Gallery ${idx}" class="w-full h-full object-cover" loading="${idx === 0 ? 'eager' : 'lazy'}" decoding="async" onerror="this.src='https://placehold.co/800x400/e2e8f0/475569?text=Gambar+Tidak+Tersedia'">
                ...
                [...filtered].reverse().forEach((row, index) => {
                    ...
                    tbody.innerHTML += `
                        <tr class="${(index+1)%2===0?'bg-slate-50/50':'bg-transparent'} hover:bg-indigo-50/60 font-mono text-[13px] border-b border-slate-200/50">
                            ...
                            <td class="px-4 py-3 text-slate-500 font-sans truncate max-w-[200px]" title="${row.alasan}">${row.alasan}</td>
                        </tr>`;
                });
            },
            exportToCSV() {
                if(state.surveys.length === 0) return system.showToast('Database kosong, tidak ada yang diekspor.', 'error');
                let csv = "ID Data,Waktu Input,Nama Siswa,Kelas,Nama Orang Tua,No. WhatsApp,Pilihan Ekstrakurikuler,Alasan Memilih\n";
                state.surveys.forEach(r => {
                    const pilihanStr = r.pilihanEkskul ? r.pilihanEkskul.join(', ') : [r.ekskul1, r.ekskul2].filter(Boolean).join(', ');
                    const row = [r.id, r.timestamp, r.nama, r.kelas, (r.namaOrtu || '-'), (r.waOrtu || '-'), pilihanStr.replace(/"/g,'""'), r.alasan.replace(/"/g,'""')];
                    csv += `"${row.join('","')}"\n`;
                });
        ...
        const cetakManager = {
            ...
            buildTabel(peserta) {
                if (peserta.length === 0) {
                    return `<tr><td colspan="4" class="text-center py-6 text-slate-400 italic">Belum ada peserta yang mendaftar.</td></tr>`;
                }
        ...
        const dashboardManager = {
            ...
            // --- Partisipasi per Kelas + daftar nama yang belum isi (actionable untuk wali kelas) ---
            ...
            // --- KPI & Kesimpulan Berbasis AI (Google Gemini) ---
            async generateAiKpi() {
                const apiKey = (state.settings && state.settings.geminiApiKey || '').trim();
                const box = document.getElementById('dash-ai-kpi-result');
                const btn = document.getElementById('dash-ai-kpi-btn');
                if (!box) return;

                if (!apiKey) {
                    box.innerHTML = `<p class="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl p-3">Gemini API Key belum diisi. Buka menu <span class="font-bold">Pengaturan</span> untuk mengisinya terlebih dahulu.</p>`;
                    return;
                }
                ...
                try {
                    ...
                    if (!res.ok) {
                        const msg = (json && json.error && json.error.message) || 'Gagal menghubungi Gemini API.';
