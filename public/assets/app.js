                    ...
                    pembimbingListEl.innerHTML = pembimbingArr.length ? pembimbingArr.map(p => {
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                ${bio ? `<p class="text-[11px] text-slate-500 leading-snug">${escapeHtml(bio)}</p>` : ''}
                            </div>
                        </div>`;
        ...
        const settingsManager = {
            ...
            save() {
                ...
                // PENTING: kalau password di atas baru saja diganti, sessionStorage
                // WAJIB ikut diperbarui dengan password yang baru. Kalau tidak,
                // request admin-only berikutnya (mis. edit Kelas/Ekskul) akan tetap
                // mengirim password LAMA lewat header X-Admin-Password -- yang sejak
                // sekarang sudah tidak cocok lagi dengan password baru di server,
                // sehingga akan ditolak (401) sampai admin logout & login ulang.
                if (state.isAdmin) {
            ...
            renderPublicList() {
                ...
                grid.innerHTML = sorted.map(g => {
                    ...
                    <div class="glass-card p-6 rounded-3xl flex flex-col items-center text-center gap-3">
                        ...
                        ${g.bio ? `<p class="text-xs text-slate-600 leading-relaxed">${escapeHtml(g.bio)}</p>` : ''}
                    </div>`;
                }).join('');
                lucide.createIcons({ root: grid });
            }
