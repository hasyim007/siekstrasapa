        ...
        // --- PROTEKSI XSS (escape HTML untuk semua data yang berasal dari isian publik) ---
        // Beberapa jenis data (survey/usulan/saran) diisi bebas oleh wali murid/siswa
        // TANPA login, lalu ditampilkan lagi ke admin (dan ke publik) lewat innerHTML.
        // Kalau teks yang mereka ketik mengandung karakter HTML (mis. "<img src=x
        // onerror=...>"), tanpa di-escape dulu, kode itu akan IKUT DIJALANKAN oleh
        // browser siapa pun yang melihat halaman itu -- termasuk browser admin.
        // Fungsi ini mengubah karakter HTML spesial jadi bentuk teks biasa (entity)
        // supaya SELALU tampil sebagai teks, tidak pernah dieksekusi sebagai kode.
        // WAJIB dipakai membungkus setiap data dari luar (survey/usulan/saran/siswa/
        // dll) sebelum disisipkan ke template innerHTML di seluruh file ini.
        ...
        // --- PROTEKSI CSV FORMULA INJECTION (untuk fitur Ekspor CSV) ---
        // Kolom seperti Nama Siswa/Nama Orang Tua/Alasan Memilih diisi bebas oleh
        // wali murid lewat form publik. Kalau isinya diawali karakter =, +, -, @
        // (atau tab/CR), Excel/Google Sheets bisa membacanya sebagai RUMUS saat
        // file CSV dibuka -- bukan sebagai teks biasa -- yang berpotensi membuka
        // link berbahaya (mis. =HYPERLINK(...)) tanpa admin sadar. Fungsi ini
        // menambahkan tanda kutip satu (') di depan nilai semacam itu supaya
        // Excel/Sheets SELALU membacanya sebagai teks apa adanya.
        ...
        // Harus SAMA PERSIS dengan ADMIN_ONLY_TYPES di worker.js -- ini daftar
        // jenis data yang sekarang wajib login (dicek server) untuk ditulis.
        // ("siswa" ditambahkan karena data ini ternyata cuma pernah diubah dari
        // halaman Master Kelas yang memang halaman admin -- lihat catatan di
        // worker.js untuk detail alasannya.)
        const ADMIN_ONLY_TYPES = ['kelas', 'ekskul', 'gallery', 'notif', 'settings', 'arsip', 'siswa', 'guru'];

        // Password admin yang berhasil login disimpan di sini (sessionStorage,
        // bukan localStorage, supaya hilang begitu tab/browser ditutup) supaya
        // bisa dikirim ulang lewat header X-Admin-Password tiap kali menyimpan
        // data admin-only. Ini menggantikan cara lama yang cuma menyimpan flag
        // boolean "isAdmin" tanpa mengirim bukti apa pun ke server.
        const ADMIN_PASS_SESSION_KEY = 'sdn01_admin_pass_session';
        ...
        const api = {
            ...
            async fetchAll() {
                ...
                try {
                    const headers = {};
                    const savedPass = sessionStorage.getItem(ADMIN_PASS_SESSION_KEY);
            ...
            push(type, data) {
                ...
                // Sertakan header X-Admin-Password kapan pun sesi admin sedang aktif,
                // TIDAK cuma untuk tipe ADMIN_ONLY_TYPES. Alasannya: tipe "publik"
                // seperti usulan/saran/survey sekarang divalidasi server dengan aturan
                // "append-only" untuk request TANPA login (lihat PUBLIC_APPEND_ONLY_TYPES
                // di worker.js) -- tapi aksi admin sendiri (hapus/setujui/tolak usulan,
                // hapus kritik-saran, edit/hapus baris spreadsheet) butuh menimpa/
                // menghapus data, jadi header ini WAJIB ikut terkirim supaya server tahu
                // permintaan ini datang dari admin yang sah, bukan dianggap publik biasa.
                const savedPass = sessionStorage.getItem(ADMIN_PASS_SESSION_KEY);
                if (savedPass) headers['X-Admin-Password'] = savedPass;
                ...
                fetch(API_URL, {
                    ...
                    if (!json || !json.success) {
                        ...
                        if (res.status === 401) {
                            // Sesi admin sudah tidak valid lagi di server (mis. password
                            // sudah diganti dari perangkat lain) -- paksa logout di sini
                            // supaya admin tidak salah kira datanya tersimpan padahal ditolak.
                            sessionStorage.removeItem(ADMIN_PASS_SESSION_KEY);
                            sessionStorage.removeItem(ADMIN_SESSION_KEY);
                            if (typeof system !== 'undefined' && system.showToast) {
                                system.showToast('Sesi admin tidak valid. Silakan login ulang, perubahan terakhir belum tersimpan di server.', 'error');
                            }
                        }
                    }
            ...
            // Versi TERTUNGGU (awaited) dari push() di atas -- dipakai KHUSUS untuk form
            // publik yang hasilnya "final" & tidak boleh diulang wali murid/siswa (survei
            // peminatan ekskul, kritik & saran): kalau push() biasa (fire-and-forget) gagal
            // karena koneksi jelek, UI tetap langsung menampilkan modal "Terima Kasih!" --
            // padahal datanya BELUM TENTU tersimpan di server. Wali murid pulang dengan
            // kesan sudah beres, padahal harus isi ulang. pushAwait() memastikan modal sukses
            // baru muncul SETELAH server benar-benar mengonfirmasi data tersimpan.
            // Mengembalikan { success, error }.
            async pushAwait(type, data) {
                if (!this.isConfigured()) return { success: false, error: 'API tidak terkonfigurasi.' };
                this.clearCache();
                ...
                try {
                    ...
                    const json = await res.json().catch(() => null);
                    if (!json || !json.success) {
                        return { success: false, error: (json && json.error) || `Server menolak permintaan (kode ${res.status}).` };
                    }
                    return { success: true };
                } catch (err) {
                    return { success: false, error: 'Tidak bisa menghubungi server. Periksa koneksi internet Anda lalu coba lagi.' };
                }
            }
        };
        ...
        const DB_KEY_GURU = 'sdn01_db_guru_v1';
        ...
        const DEFAULT_SETTINGS = {
            ...
            guruHeroDescription: 'Daftar guru dan pelatih yang membina kegiatan ekstrakurikuler di SDN 01 PAPAHAN -- lengkap dengan foto dan bio singkat, supaya wali murid lebih mengenal siapa yang membimbing putra-putrinya.',
        ...
        const state = {
            ...
            saran: [],
            arsip: [],
            // Master Data Guru/Pelatih -- item: { id, nama, foto (URL/data URL, opsional), bio (1-2 kalimat, opsional) }
            guru: [],
            activePage: 'beranda',
            isAdmin: false,
        ...
        const system = {
            async init() {
                ...
                // tendang ke halaman Login (bukan Beranda), sambil membawa nama file
                // halaman yang tadi dituju lewat ?redirect= supaya setelah login berhasil
                // langsung dibawa ke halaman itu lagi. (Catatan: ini proteksi sisi tampilan
                // saja, sama seperti versi SPA sebelumnya -- bukan keamanan server-side yang
                // sesungguhnya; keamanan sesungguhnya tetap dari checkAdminAuth di worker.js.)
                if (ADMIN_PAGES.includes(currentPage) && !state.isAdmin) {
                    const targetFile = PAGE_FILES[currentPage] || 'dashboard.html';
                    window.location.href = 'login.html?redirect=' + encodeURIComponent(targetFile);
                    return;
                }
                ...
                if (remote) {
                    ...
                    state.guru = JSON.parse(localStorage.getItem(DB_KEY_GURU)) || [];
            ...
            applyBranding() {
                ...
                const guruHeroDescEl = document.getElementById('guru-hero-description');
                if (guruHeroDescEl) guruHeroDescEl.textContent = s.guruHeroDescription || DEFAULT_SETTINGS.guruHeroDescription;
            ...
            saveData(type, syncRemote = true) {
                ...
                if(type === 'guru' || type === 'all') { localStorage.setItem(DB_KEY_GURU, JSON.stringify(state.guru)); if(syncRemote) api.push('guru', state.guru); }
            },
            ...
            // Ringkas daftar kelas paralel jadi kelas utama saja untuk tampilan kartu yang padat,
            // mis. ["2A","2B","2C","3A","3B"] -> "2, 3" (bukan "2A, 2B, 2C, 3A, 3B" yang kepanjangan
            // dan bikin baris jadwal di sebelahnya kepotong). Detail lengkap per kelas paralel tetap
            // bisa dilihat wali murid lewat "Lihat Selengkapnya" (modal detail tidak diringkas).
            simplifyKelasLabel(kelasArr) {
                if (!kelasArr.length) return 'Semua Kelas';
                const grades = new Set();
                let allNumeric = true;
                kelasArr.forEach(k => {
                    const m = String(k).trim().match(/^(\d+)/);
                    if (m) grades.add(Number(m[1])); else allNumeric = false;
                });
                if (!allNumeric || grades.size === 0) return kelasArr.join(', ');
                return 'Kelas ' + Array.from(grades).sort((a, b) => a - b).join(', ');
            },
        ...
        const PAGE_FILES = {
            ...
            // "guru" = Master Data Guru/Pelatih (halaman admin, mirip Master Kelas).
            // "gurupublik" = halaman "Guru & Pelatih" (publik, tanpa login, buat dipamerkan
            // ke wali murid baru/PPDB) -- SENGAJA TIDAK dimasukkan ke ADMIN_PAGES di bawah.
            guru: 'guru.html', gurupublik: 'guru-publik.html'
        };
        // Halaman yang hanya boleh diakses setelah login admin
        const ADMIN_PAGES = ['dashboard', 'kelas', 'kelola', 'usulan', 'galeri', 'siswa', 'spreadsheet', 'pengaturan', 'cetak', 'notifikasi', 'saran', 'guru'];
        ...
        const appController = {
            ...
            renderPage(targetPageId) {
                ...
                ['beranda', 'survey', 'tutorial', 'gurupublik', 'dashboard', 'kelas', 'guru', 'kelola', 'usulan', 'galeri', 'siswa', 'spreadsheet', 'pengaturan', 'cetak', 'notifikasi', 'saran'].forEach(id => {
                ...
                try {
                    ...
                    if(targetPageId === 'guru') teacherManager.renderTable();
                    if(targetPageId === 'gurupublik') teacherManager.renderPublicList();
        ...
        const uiManager = {
            ...
            renderBerandaCards() {
                ...
                // Elemen ini cuma ada di index.html (halaman Beranda). Kalau function ini
                // dipanggil dari halaman lain (mis. survey.html setelah submit), container
                // akan null -- keluar lebih awal supaya tidak crash (innerHTML on null).
                if (!container) return;
                ...
                container.innerHTML = list.map(item => {
                    ...
                    if (penuh) {
                        ...
                        statusBadge = `<span class="px-2.5 py-1 text-[10px] font-bold ${hampirPenuh ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'} rounded-lg uppercase">Sisa ${sisa} Kuota</span>`;
                    } else {
                    ...
                    const terdaftarInfo = `<p class="text-[10px] text-slate-500 font-medium mb-3">${terisi} siswa telah mendaftar${kuota > 0 ? ` dari ${kuota} kuota` : ''}</p>`;

                    // Kartu per kelompok kelas & hari. Kelas paralel diringkas jadi kelas utama saja
                    // (mis. "2A, 2B, 2C" -> "Kelas 2") dan jadwal dipisah ke baris sendiri (bukan
                    // digabung satu baris dengan kelas) supaya tidak kepotong di kartu yang sempit.
                    // Detail lengkap per kelas paralel tetap ada di modal "Lihat Selengkapnya".
                    const grupBadges = grupArr.length ? `<div class="space-y-2 mb-4">${grupArr.map(g => {
                        const kelasArr = system.toArr(g.kelas);
                        const kelasLabel = system.simplifyKelasLabel(kelasArr);
                        return `<div class="px-2.5 py-1.5 bg-slate-100 rounded-lg border border-slate-200 text-slate-600">
                            ${g.keterangan ? `<p class="text-[10px] font-bold text-slate-700 mb-1">${g.keterangan}</p>` : ''}
                            <p class="flex items-center gap-1.5 text-[10px] font-semibold"><i data-lucide="users-round" class="w-3 h-3 flex-shrink-0"></i> <span>${kelasLabel}</span></p>
                            ${g.hari ? `<p class="flex items-center gap-1.5 text-[10px] font-semibold mt-1"><i data-lucide="clock" class="w-3 h-3 flex-shrink-0"></i> <span>${g.hari}</span></p>` : ''}
                        </div>`;
                    }).join('')}</div>` : '';
                    // Foto pembina/pelatih diambil dari Master Data Guru/Pelatih (sama seperti
                    // di modal detail) -- kalau ID pembimbing teks bebas peninggalan data lama
                    // (tidak ditemukan di master), tetap tampilkan namanya saja pakai ikon default.
                    const pembimbingHtml = pembimbingArr.length ? pembimbingArr.map(p => {
                        const guru = teacherManager.findById(p);
                        const namaGuru = guru ? guru.nama : String(p);
                        const fotoUrl = guru && guru.foto ? system.normalizeImageUrl(guru.foto) : '';
                        return `
                        <div class="flex items-center gap-2 min-w-0">
                            <div class="w-7 h-7 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden flex-shrink-0 border border-slate-200">
                                ${fotoUrl ? `<img src="${fotoUrl}" alt="${escapeHtml(namaGuru)}" class="w-full h-full object-cover" loading="lazy" decoding="async" onerror="this.parentElement.innerHTML='<i data-lucide=&quot;user&quot; class=&quot;w-3.5 h-3.5 text-slate-500&quot;></i>'">` : `<i data-lucide="user" class="w-3.5 h-3.5 text-slate-500"></i>`}
                            </div>
                            <span class="text-xs font-semibold text-slate-800 truncate">${escapeHtml(namaGuru)}</span>
                        </div>`;
                    }).join('') : `
                        <div class="flex items-center gap-2">
                            <div class="w-7 h-7 bg-slate-200 rounded-full flex items-center justify-center flex-shrink-0"><i data-lucide="user" class="w-3.5 h-3.5 text-slate-500"></i></div>
                            <span class="text-xs font-semibold text-slate-800">-</span>
                        </div>`;
                    return `
                    <div class="glass-card p-5 md:p-6 rounded-2xl h-full flex flex-col justify-between group">
                        <div>
                            ...
                            <p class="text-xs text-slate-600 line-clamp-3 mb-2 leading-relaxed text-justify">${item.deskripsi}</p>
                        ...
                        <div class="pt-4 border-t border-slate-200/60 mt-auto">
                            <p class="text-[9px] uppercase font-bold text-slate-400 mb-1.5">Pembina / Pelatih</p>
                            <div class="space-y-1.5">${pembimbingHtml}</div>
                        </div>
                    </div>`;
                }).join('');
            ...
            populateEkskulCheckboxes() {
                ...
                if(!container) return;
                const emptyHint = document.getElementById('sv-ekskul-empty-hint');

                // Saring ekskul sesuai kelas yang sedang dipilih di form, berdasarkan
                // settingan kelas per kelompok di halaman Kelola. Kelompok yang kelasnya
                // dikosongkan (belum diatur) dianggap berlaku untuk semua kelas, supaya
                // ekskul lama yang belum di-setting ulang tidak mendadak hilang.
                const kelasSelect = document.getElementById('sv-kelas');
                const kelasVal = kelasSelect ? kelasSelect.value : '';
                const ekskulTersedia = !kelasVal ? state.ekskul : state.ekskul.filter(ek => {
                    const grupArr = system.toGrupArr(ek);
                    if(!grupArr.length) return true; // ekskul lama tanpa kelompok sama sekali
                    return grupArr.some(g => {
                        const kelasArr = system.toArr(g.kelas);
                        return kelasArr.length === 0 || kelasArr.includes(kelasVal);
                    });
                });

                if(emptyHint) emptyHint.classList.toggle('hidden', ekskulTersedia.length > 0 || !kelasVal);

                container.innerHTML = ekskulTersedia.map(ek => {
                    ...
                    const sisaBadge = kuota > 0
                        ? `<span class="text-[10px] font-bold ${penuh ? 'text-rose-600' : 'text-emerald-600'}">${penuh ? 'Kuota penuh' : `Sisa ${kuota - terisi} kuota`}</span>`
                        : '';
                    return `
            ...
            openEkskulDetailModal(id) {
                ...
                // Foto + bio singkat tiap pembimbing (dari Master Data Guru/Pelatih) -- kalau
                // ID pembimbing ternyata teks bebas peninggalan data lama (tidak ditemukan di
                // master), tetap tampilkan namanya saja tanpa foto/bio supaya tidak hilang.
                const pembimbingListEl = document.getElementById('detail-ekskul-pembimbing-list');
                if (pembimbingListEl) {
                    pembimbingListEl.innerHTML = pembimbingArr.length ? pembimbingArr.map(p => {
                        const guru = teacherManager.findById(p);
                        const nama = guru ? guru.nama : String(p);
                        const fotoUrl = guru && guru.foto ? system.normalizeImageUrl(guru.foto) : '';
                        const bio = guru && guru.bio ? guru.bio : '';
                        return `
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden flex-shrink-0 border border-slate-200">
                                ${fotoUrl ? `<img src="${fotoUrl}" alt="${escapeHtml(nama)}" class="w-full h-full object-cover" loading="lazy" decoding="async" onerror="this.parentElement.innerHTML='<i data-lucide=&quot;user&quot; class=&quot;w-4 h-4 text-slate-500&quot;></i>'">` : `<i data-lucide="user" class="w-4 h-4 text-slate-500"></i>`}
                            </div>
                            <div class="min-w-0">
                                <p class="text-xs font-bold text-slate-800 truncate">${escapeHtml(nama)}</p>
                                ${bio ? `<p class="text-[11px] text-slate-500 leading-snug text-justify">${escapeHtml(bio)}</p>` : ''}
                            </div>
                        </div>`;
                    }).join('') : '<p class="text-xs text-slate-500">-</p>';
                    lucide.createIcons({ root: pembimbingListEl });
                }
        ...
        const settingsManager = {
            ...
            renderForm() {
                ...
                document.getElementById('setting-guru-hero-desc').value = s.guruHeroDescription || '';
            ...
            async save() {
                ...
                const guruHeroDescEl = document.getElementById('setting-guru-hero-desc');
                const guruHeroDesc = guruHeroDescEl ? guruHeroDescEl.value.trim() : (state.settings.guruHeroDescription || '');
                ...
                // Password/username LAMA (sebelum diganti) -- disimpan dulu di sini supaya
                // kalau ternyata request ke server gagal di bawah, sessionStorage TIDAK ikut
                // diperbarui dan admin tetap bisa lanjut pakai kredensial lama tanpa perlu
                // login ulang (lihat penjelasan lebih lengkap di komentar dekat pushAwait di bawah).
                const passwordLama = state.settings.adminPassword || DEFAULT_SETTINGS.adminPassword;
                ...
                state.settings = {
                    ...
                    guruHeroDescription: guruHeroDesc || DEFAULT_SETTINGS.guruHeroDescription,
                ...
                // Simpan ke localStorage (cache offline) dulu seperti biasa, TAPI pengiriman
                // ke server sengaja DITUNGGU (pushAwait, bukan saveData/push yang fire-and-
                // forget) khusus untuk "settings" -- soalnya blob ini berisi adminPassword.
                // Kalau kita langsung update sessionStorage dengan password baru padahal
                // request ke server BELUM TENTU berhasil (mis. koneksi terputus), request
                // admin-only berikutnya (edit Kelas/Ekskul, dst) akan memakai password baru
                // yang ternyata tidak cocok dengan yang tersimpan di server -> admin ditolak
                // (401) terus sampai logout & login ulang, padahal dia tidak sadar sudah
                // "ter-logout" secara diam-diam.
                localStorage.setItem(DB_KEY_SETTINGS, JSON.stringify([state.settings]));
                system.applyBranding();

                const result = await api.pushAwait('settings', [state.settings]);

                if (!result.success) {
                    system.showToast('Gagal menyimpan pengaturan ke server: ' + result.error + ' Perubahan belum tersimpan permanen -- silakan coba "Simpan" lagi.', 'error');
                    return;
                }

                // Sukses & terkonfirmasi server -> baru sekarang aman memperbarui sesi admin
                // (kalau password memang baru saja diganti).
                if (state.isAdmin && state.settings.adminPassword !== passwordLama) {
                    sessionStorage.setItem(ADMIN_PASS_SESSION_KEY, state.settings.adminPassword);
                }
        ...
        const arsipManager = {
            ...
            exportCSV(id) {
                ...
                arsip.surveys.forEach(r => {
                    ...
                    const row = [r.id, r.timestamp, r.nama, r.kelas, jkLabel, (r.namaOrtu || '-'), (r.waOrtu || '-'), pilihanStr, (r.alasan || '')]
        ...
        const authManager = {
            async login(e) {
                ...
                // Username cuma pemeriksaan kosmetik di sisi tampilan (field-nya memang
                // disembunyikan & selalu "admin") -- yang jadi PENENTU SAH/TIDAKNYA login
                // sekarang adalah verifikasi password ke server lewat /api/login, BUKAN lagi
                // dibandingkan langsung di browser terhadap state.settings.adminPassword
                // (yang sejak perbaikan worker.js, memang tidak lagi dikirim ke browser
                // sebelum login berhasil).
                ...
                try {
                    ...
                    if (json && json.success) {
                        ...
                        // Simpan password yang barusan terbukti valid, supaya bisa dikirim
                        // ulang lewat header X-Admin-Password tiap kali menyimpan data
                        // admin-only (lihat api.push di atas).
                        sessionStorage.setItem(ADMIN_PASS_SESSION_KEY, enteredPass);
                        uiManager.toggleLoginModal(false);
                        system.applyAdminUI(true);
                        system.showToast('Login Administrator Berhasil', 'success');
                        // Muat ulang data dari server sambil membawa password admin, supaya
                        // field seperti password tersimpan di halaman Pengaturan ikut terisi.
                        api.clearCache();
                        appController.switchPage('dashboard');
                    } else {
                        const errEl = document.getElementById('login-error-msg');
                        // Tampilkan pesan asli dari server kalau ada (mis. pesan rate-limit
                        // "terlalu banyak percobaan, coba lagi dalam X menit"), bukan cuma
                        // teks statis "Kredensial tidak valid" supaya pengguna tahu alasan
                        // sebenarnya kalau kena limit percobaan login.
                        errEl.textContent = (json && json.error) ? json.error : 'Kredensial tidak valid.';
                        errEl.classList.remove('d-none');
                    }
                } catch (err) {
                    system.showToast('Gagal menghubungi server untuk login. Cek koneksi internet Anda.', 'error');
                } finally {
        ...
        const surveyManager = {
            ...
            onKelasChange(kelasVal) {
                ...
                // Sesuaikan pilihan ekstrakurikuler yang tampil dengan kelas yang baru dipilih
            ...
            async submitSurvey(e) {
                ...
                // Cek apakah siswa sudah mengisi (pengecekan awal, masih pakai data yang
                // sudah dimuat di browser -- pengecekan FINAL & yang benar-benar menentukan
                // ada di bawah, setelah data terbaru dari server diambil ulang).
                ...
                // --- Mulai proses simpan: kunci tombol supaya tidak ke-klik dobel, dan
                //     kasih tahu wali murid kalau sedang proses (bukan langsung "sukses"). ---
                const submitBtn = e.target.querySelector('button[type="submit"]');
                const submitBtnHtmlAsal = submitBtn ? submitBtn.innerHTML : '';
                if (submitBtn) {
                ...
                try {
                    // Ambil ULANG data terbaru langsung dari server (bukan cache/state lama di
                    // browser) sebelum benar-benar menyimpan. Ini memperkecil celah race-condition:
                    // kalau ada wali murid LAIN yang baru saja submit untuk siswa yang sama, atau
                    // kuota ekskul baru saja penuh, beberapa detik terakhir sebelum form ini dibuka,
                    // kita perlu tahu itu SEBELUM ikut menambahkan data yang salah/duplikat.
                    api.clearCache();
                    const fresh = await api.fetchAll();
                    if (fresh) {
                        state.surveys = fresh.surveys || [];
                        if (Array.isArray(fresh.ekskul)) state.ekskul = fresh.ekskul;
                    }

                    // Cek ulang: siswa sudah isi? (final check, pakai data terbaru dari server)
                    if (state.surveys.some(s => s.idSiswa === idSiswa)) {
                        system.showToast('Siswa ini baru saja terdaftar (mungkin oleh wali murid lain). Silakan cek kembali, tidak perlu isi ulang.', 'error');
                        return;
                    }

                    // Cek ulang kesesuaian kelas (final check, pakai data terbaru dari server).
                    // Jaring pengaman untuk kasus: tab formulir sudah lama terbuka lalu admin
                    // ubah settingan kelas di ekskul terkait, atau ada yang mencoba mengaktifkan
                    // ulang checkbox lewat DevTools. Aturan sama seperti filter tampilan: ekskul
                    // dianggap valid untuk kelas siswa kalau ada kelompok yang cocok kelasnya,
                    // ATAU ekskul itu memang tidak punya kelompok/batasan kelas sama sekali.
                    for (const eksName of selectedEkskul) {
                        const eksObj = state.ekskul.find(ek => ek.nama === eksName);
                        if (!eksObj) continue; // ekskul usulan baru (belum ada di master) ditangani terpisah di bawah
                        const grupArr = system.toGrupArr(eksObj);
                        const kelasCocok = !grupArr.length || grupArr.some(g => {
                            const kelasArr = system.toArr(g.kelas);
                            return kelasArr.length === 0 || kelasArr.includes(siswaObj.kelas);
                        });
                        if (!kelasCocok) {
                            uiManager.populateEkskulCheckboxes();
                            system.showToast(`Ekstrakurikuler "${eksName}" tidak tersedia untuk kelas ${siswaObj.kelas}. Silakan pilih ulang.`, 'error');
                            return;
                        }
                    }

                    // Cek ulang kuota (final check, pakai data terbaru dari server)
                    for (const eksName of selectedEkskul) {
                        const eksObj = state.ekskul.find(ek => ek.nama === eksName);
                        ...
                        if (kuota > 0) {
                            ...
                            if (terisi >= kuota) {
                                uiManager.populateEkskulCheckboxes();
                                system.showToast(`Mohon maaf, kuota "${eksName}" baru saja penuh. Silakan pilih ekstrakurikuler lain lalu kirim ulang.`, 'error');
                                return;
                            }
                        }
                    }
                    ...
                    const surveysBaru = state.surveys.concat([payload]);

                    // KIRIM & TUNGGU konfirmasi server -- baru lanjut ke modal sukses kalau
                    // server benar-benar bilang "success". Kalau gagal (jaringan putus, server
                    // menolak, dsb), TIDAK reset form supaya wali murid tidak perlu mengetik ulang,
                    // cukup klik "Daftarkan" lagi setelah baca pesan error di bawah.
                    const result = await api.pushAwait('survey', surveysBaru);
                    if (!result.success) {
                        system.showToast('Gagal menyimpan: ' + result.error + ' Isian Anda tidak hilang -- silakan coba kirim lagi.', 'error');
                        return;
                    }

                    // Sukses & terkonfirmasi server -> baru update state lokal + localStorage.
                    state.surveys = surveysBaru;
                    localStorage.setItem(DB_KEY_SURVEY, JSON.stringify(state.surveys));
                    ...
                    // (Usulan sifatnya "bonus", jadi kalau ini gagal simpan, tetap dianggap survei
                    // utama berhasil -- cukup beri tahu lewat toast, jangan blok modal sukses survei.)
                    let usulanTerkirim = false;
                    if(usulanAktif && usulanNama !== '') {
                        const usulanBaru = state.usulan.concat([{
                        ...
                        const usulanResult = await api.pushAwait('usulan', usulanBaru);
                        if (usulanResult.success) {
                            state.usulan = usulanBaru;
                            localStorage.setItem(DB_KEY_USULAN, JSON.stringify(state.usulan));
                            usulanManager.updateBadge();
                            usulanTerkirim = true;
                        } else {
                            system.showToast('Pendaftaran ekskul berhasil, tapi usulan ekskul baru gagal terkirim. Bisa disampaikan lagi lewat "Saran & Masukan".', 'error');
                        }
                    }
                    ...
                    // JARING PENGAMAN: sebelumnya blok ini TIDAK ADA (cuma try/finally),
                    // jadi kalau ada error tak terduga di tengah proses (bukan error
                    // "gagal menyimpan" biasa yang sudah ditangani lewat return di atas),
                    // errornya hilang begitu saja -- tombol kembali normal tapi wali murid
                    // TIDAK melihat pesan apa pun (sukses maupun gagal). Sekarang errornya
                    // selalu ditangkap dan ditampilkan lewat toast supaya wali murid tahu
                    // pendaftarannya belum tersimpan dan perlu mencoba lagi / lapor ke admin.
                    console.error('submitSurvey error:', err);
                    system.showToast('Terjadi kesalahan tak terduga: ' + (err && err.message ? err.message : err) + '. Isian Anda tidak hilang -- silakan coba kirim lagi.', 'error');
                } finally {
                    if (submitBtn) {
                        submitBtn.disabled = false;
                        submitBtn.innerHTML = submitBtnHtmlAsal;
                        lucide.createIcons({ root: submitBtn });
                    }
                }
            }
        ...
        const classManager = {
            // Menyimpan id kelas yang sedang dibuka di modal "Kelola Siswa" (Detail Kelas),
            // supaya fungsi add/edit/delete siswa di bawah tahu kelas mana yang aktif.
            _activeKelasId: null,
            ...
            renderTable() {
                ...
                if(state.kelas.length === 0) {
                    grid.innerHTML = `<div class="col-span-full p-10 text-center bg-white/40 rounded-2xl border border-dashed border-slate-300 text-slate-500 text-sm">Belum ada kelas yang terdaftar. Klik "Tambah Kelas" atau "Import Excel/CSV" di atas untuk mulai.</div>`;
                    return;
                }
                ...
                sorted.forEach((item) => {
                    ...
                    grid.innerHTML += `
                        <div class="glass-card rounded-2xl p-5 cursor-pointer hover:shadow-lg hover:border-indigo-200 transition-all group" onclick="classManager.openSiswaKelasModal('${item.id}')">
                            <div class="flex items-start justify-between gap-2 mb-4">
                                <div class="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                                    <i data-lucide="door-open" class="w-5 h-5"></i>
                                </div>
                                <div class="flex items-center gap-1" onclick="event.stopPropagation()">
                                    <button onclick="uiManager.openKelasModal('${item.id}')" class="text-slate-400 hover:text-indigo-600 bg-white p-1.5 border rounded-lg shadow-sm" title="Edit Nama Kelas"><i data-lucide="pencil" class="w-3.5 h-3.5"></i></button>
                                    <button onclick="classManager.deleteKelas('${item.id}', '${item.nama}')" class="text-slate-400 hover:text-rose-600 bg-white p-1.5 border rounded-lg shadow-sm" title="Hapus Kelas"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button>
                                </div>
                            </div>
                            <h4 class="font-bold text-slate-800 text-base mb-1 truncate">${item.nama}</h4>
                            <p class="text-xs text-slate-500">${studentCount} siswa terdaftar</p>
                            ${item.waWaliKelas ? `<p class="text-[10px] text-emerald-600 font-semibold mt-2 flex items-center gap-1"><i data-lucide="message-circle" class="w-3 h-3"></i> WA wali kelas tersimpan</p>` : ''}
                            <p class="text-[10px] text-indigo-500 font-semibold mt-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><i data-lucide="mouse-pointer-click" class="w-3 h-3"></i> Klik untuk kelola siswa</p>
                        </div>
                    `;
                });
            ...
            // ===== MODAL "KELOLA SISWA" PER KARTU KELAS =====
            // Dibuka dengan klik body kartu kelas (bukan tombol pensil/hapus kelas yang di
            // pojok kanan atas, itu sudah di-stopPropagation di renderTable di atas).
            openSiswaKelasModal(kelasId) {
                const kelas = state.kelas.find(k => k.id === kelasId);
                if (!kelas) return;
                this._activeKelasId = kelasId;

                const m = document.getElementById('modal-siswa-kelas'), c = document.getElementById('modal-siswa-kelas-card');
                m.classList.remove('pointer-events-none'); void m.offsetWidth; m.classList.add('opacity-100'); c.classList.remove('scale-95');

                document.getElementById('siswa-kelas-new-nama').value = '';
                this.renderSiswaKelasList();
                setTimeout(() => document.getElementById('siswa-kelas-new-nama').focus(), 150);
            },

            closeSiswaKelasModal() {
                const m = document.getElementById('modal-siswa-kelas'), c = document.getElementById('modal-siswa-kelas-card');
                m.classList.remove('opacity-100'); c.classList.add('scale-95'); setTimeout(() => m.classList.add('pointer-events-none'), 300);
                this._activeKelasId = null;
            },

            renderSiswaKelasList() {
                const kelas = state.kelas.find(k => k.id === this._activeKelasId);
                if (!kelas) { this.closeSiswaKelasModal(); return; }

                const listEl = document.getElementById('siswa-kelas-list');
                const filtered = state.siswa.filter(s => s.kelas === kelas.nama).sort((a,b) => a.nama.localeCompare(b.nama));

                document.getElementById('siswa-kelas-title').innerText = `Siswa Kelas ${kelas.nama}`;
                document.getElementById('siswa-kelas-subtitle').innerText = `${filtered.length} siswa terdaftar`;

                if (filtered.length === 0) {
                    listEl.innerHTML = `<p class="text-center text-xs text-slate-400 py-8">Belum ada siswa di kelas ini. Tambahkan lewat kotak di atas.</p>`;
                    return;
                }

                listEl.innerHTML = filtered.map(s => `
                    <div class="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100" data-siswa-row="${s.id}">
                        <span class="flex-1 text-sm text-slate-700 truncate siswa-view-text">${s.nama}</span>
                        <input type="text" value="${s.nama}" class="hidden flex-1 px-2.5 py-1.5 rounded-lg border border-indigo-300 text-sm siswa-edit-input" onkeydown="if(event.key==='Enter'){event.preventDefault(); classManager.saveEditSiswa('${s.id}');} else if(event.key==='Escape'){classManager.renderSiswaKelasList();}">
                        <button onclick="classManager.startEditSiswa('${s.id}')" class="siswa-view-btn text-slate-400 hover:text-indigo-600 p-1.5 flex-shrink-0" title="Edit nama"><i data-lucide="pencil" class="w-3.5 h-3.5"></i></button>
                        <button onclick="classManager.saveEditSiswa('${s.id}')" class="hidden siswa-edit-btn text-emerald-600 hover:text-emerald-700 p-1.5 flex-shrink-0" title="Simpan"><i data-lucide="check" class="w-3.5 h-3.5"></i></button>
                        <button onclick="classManager.deleteSiswaFromKelas('${s.id}')" class="siswa-view-btn text-slate-400 hover:text-rose-600 p-1.5 flex-shrink-0" title="Hapus"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button>
                    </div>
                `).join('');
            ...
            addSiswaToKelas() {
                const kelas = state.kelas.find(k => k.id === this._activeKelasId);
                if (!kelas) return;
                const input = document.getElementById('siswa-kelas-new-nama');
                const nama = input.value.trim();
                if (!nama) return;

                if (state.siswa.some(s => s.nama.toLowerCase() === nama.toLowerCase() && s.kelas === kelas.nama)) {
                    system.showToast(`"${nama}" sudah terdaftar di kelas ${kelas.nama}.`, 'error');
                    return;
                }

                state.siswa.push({ id: system.generateId('sw'), nama, kelas: kelas.nama });
                system.saveData('siswa');
                input.value = '';
                input.focus();
                this.renderSiswaKelasList();
                this.renderTable(); // supaya jumlah siswa di kartu belakang modal ikut update
                if (state.activePage === 'siswa') studentManager.renderTable();
                system.showToast(`"${nama}" ditambahkan ke kelas ${kelas.nama}.`, 'success');
            },

            startEditSiswa(id) {
                const row = document.querySelector(`[data-siswa-row="${id}"]`);
                if (!row) return;
                row.querySelector('.siswa-view-text').classList.add('hidden');
                row.querySelector('.siswa-edit-input').classList.remove('hidden');
                row.querySelectorAll('.siswa-view-btn').forEach(b => b.classList.add('hidden'));
                row.querySelector('.siswa-edit-btn').classList.remove('hidden');
                const inputEl = row.querySelector('.siswa-edit-input');
            ...
            saveEditSiswa(id) {
                ...
                const newNama = row.querySelector('.siswa-edit-input').value.trim();
                if (!newNama) { system.showToast('Nama tidak boleh kosong.', 'error'); return; }
                if (state.siswa.some(s => s.id !== id && s.nama.toLowerCase() === newNama.toLowerCase() && s.kelas === kelas.nama)) {
                    system.showToast(`"${newNama}" sudah ada di kelas ini.`, 'error'); return;
                }

                const siswaObj = state.siswa.find(s => s.id === id);
                if (siswaObj) siswaObj.nama = newNama;
                system.saveData('siswa');
                this.renderSiswaKelasList();
                if (state.activePage === 'siswa') studentManager.renderTable();
                system.showToast('Nama siswa diperbarui.', 'success');
            },

            deleteSiswaFromKelas(id) {
                const siswaObj = state.siswa.find(s => s.id === id);
                if (!siswaObj) return;
                uiManager.confirm(`Hapus "${siswaObj.nama}" dari kelas ini?`, () => {
                    state.siswa = state.siswa.filter(s => s.id !== id);
                    system.saveData('siswa');
                    this.renderSiswaKelasList();
                    this.renderTable();
                    if (state.activePage === 'siswa') studentManager.renderTable();
                    system.showToast('Siswa dihapus.', 'success');
                });
            },
            ...
            // ===== IMPORT KELAS & SISWA DARI EXCEL/CSV =====
            // Format file yang diharapkan: 2 kolom "Kelas" & "Nama Siswa", 1 baris = 1 siswa.
            // Ini dipisah dari fitur "paste per kelas" yang sudah ada di modal Tambah/Edit Kelas,
            // karena tujuannya beda: import ini untuk isi SEMUA kelas (1-6) sekaligus dari 1 file,
            // bukan 1 kelas per submit.
            _pendingImportRows: [], // hasil parse file, menunggu dikonfirmasi user di layar preview

            openBulkImportModal() {
                const m = document.getElementById('modal-kelas-import'), c = document.getElementById('modal-kelas-import-card');
                m.classList.remove('pointer-events-none'); void m.offsetWidth; m.classList.add('opacity-100'); c.classList.remove('scale-95');
                // Selalu reset ke langkah 1 (pilih file) tiap kali modal dibuka
                document.getElementById('kelas-import-file').value = '';
                document.getElementById('import-file-error').classList.add('hidden');
                document.getElementById('import-step-format').classList.remove('hidden');
                document.getElementById('import-step-preview').classList.add('hidden');
                this._pendingImportRows = [];
            },
            ...
            downloadImportTemplate() {
                ...
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url; a.download = 'template-import-kelas-siswa.csv';
                document.body.appendChild(a); a.click(); document.body.removeChild(a);
                URL.revokeObjectURL(url);
            },

            // Parser CSV sederhana dengan AUTO-DETEKSI pemisah kolom (koma / titik koma /
            // tab), dibaca dari baris pertama (header). PENTING: banyak Excel versi
            // Indonesia & Eropa memakai TITIK KOMA (;) sebagai pemisah CSV -- bukan koma
            // (,) seperti versi Excel Amerika -- karena di region itu koma dipakai sebagai
            // pemisah desimal angka. Kalau parser dipaksa selalu split pakai koma, file
            // seperti itu akan terbaca sebagai 1 kolom besar saja: header "Kelas;Nama
            // Siswa" dianggap 1 nama kolom, sehingga nama kelas & nama siswa tergabung
            // jadi satu string seperti "1A;Ahmad Budi" -- ini persis bug yang bikin
            // hasil import jadi 1 "kelas" per siswa alih-alih dikelompokkan per kelas.
            // Menangani quote ganda ("...") kalau ada, karena Excel kadang membungkus
            // nama yang mengandung koma/titik-koma dengan tanda kutip saat "Save As CSV".
            ...
            handleImportFile(event) {
                ...
                const reader = new FileReader();
                reader.onerror = () => showError('Gagal membaca file. Coba upload ulang.');

                if (isCSV) {
                    reader.onload = (e) => {
                        try {
                            const rows = this._parseCSV(String(e.target.result));
                            this._processImportRows(rows, showError);
                        } catch (err) { showError('Gagal membaca isi file CSV: ' + err.message); }
                    };
                    reader.readAsText(file, 'utf-8');
                } else if (isExcel) {
                    if (typeof XLSX === 'undefined') {
                        showError('Pembaca file Excel gagal dimuat (mungkin diblokir jaringan sekolah). Silakan simpan file sebagai .csv lalu upload ulang.');
                        return;
                    }
                    reader.onload = (e) => {
                        try {
                            ...
                            this._processImportRows(rows, showError);
                        } catch (err) { showError('Gagal membaca isi file Excel: ' + err.message); }
                    };
                    reader.readAsArrayBuffer(file);
                } else {
                    showError('Format file tidak dikenali. Gunakan file .xlsx, .xls, atau .csv.');
                }
            },

            // Validasi header + susun rows jadi { kelas -> [nama, ...] }, lalu tampilkan preview
            _processImportRows(rows, showError) {
                if (!rows.length) { showError('File kosong, tidak ada data yang bisa dibaca.'); return; }

                let [header, ...dataRows] = rows;
                const kelasColIdx = header.findIndex(h => h.toLowerCase().includes('kelas'));
                const namaColIdx = header.findIndex(h => h.toLowerCase().includes('nama'));

                if (kelasColIdx === -1 || namaColIdx === -1) {
                    showError('Kolom "Kelas" dan/atau "Nama Siswa" tidak ditemukan di baris pertama. Pastikan baris pertama file adalah judul kolom, sesuai contoh format di atas.');
                    return;
                }

                const grouped = {}; // { "1A": ["Ahmad Budi", ...] }
                let totalSiswa = 0, skippedEmpty = 0;
                dataRows.forEach(row => {
                    const kelasNama = (row[kelasColIdx] || '').trim();
                    const siswaNama = (row[namaColIdx] || '').trim();
                    if (!kelasNama || !siswaNama) { skippedEmpty++; return; }
                    if (!grouped[kelasNama]) grouped[kelasNama] = [];
                    grouped[kelasNama].push(siswaNama);
                    totalSiswa++;
                });

                if (totalSiswa === 0) {
                    showError('Tidak ada baris siswa yang valid (kolom Kelas atau Nama Siswa kosong di semua baris).');
                    return;
                }

                this._pendingImportRows = grouped;
                this._renderImportPreview(grouped, totalSiswa, skippedEmpty);
            },

            _renderImportPreview(grouped, totalSiswa, skippedEmpty) {
                const kelasNames = Object.keys(grouped).sort((a, b) => a.localeCompare(b));
                const existingKelasNames = new Set(state.kelas.map(k => k.nama.toLowerCase()));

                document.getElementById('import-preview-summary').innerHTML =
                    `${kelasNames.length} kelas terdeteksi &bull; ${totalSiswa} siswa siap diimpor` +
                    (skippedEmpty > 0 ? ` <span class="font-normal">(${skippedEmpty} baris kosong/tidak lengkap dilewati)</span>` : '');

                const listEl = document.getElementById('import-preview-list');
                listEl.innerHTML = kelasNames.map(kelasNama => {
                    const siswaArr = grouped[kelasNama];
                    const isBaru = !existingKelasNames.has(kelasNama.toLowerCase());
                    return `<div class="px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl">
                        <p class="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                            ${kelasNama}
                            <span class="text-[9px] font-semibold px-1.5 py-0.5 rounded ${isBaru ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200 text-slate-600'}">${isBaru ? 'Kelas Baru' : 'Kelas Sudah Ada'}</span>
                        </p>
                        <p class="text-[10px] text-slate-500 mt-1">${siswaArr.length} siswa</p>
                    </div>`;
                }).join('');

                document.getElementById('import-step-format').classList.add('hidden');
                document.getElementById('import-step-preview').classList.remove('hidden');
                lucide.createIcons({root: document.getElementById('modal-kelas-import-card')});
            },

            cancelBulkImportPreview() {
                this._pendingImportRows = [];
                document.getElementById('kelas-import-file').value = '';
                document.getElementById('import-step-preview').classList.add('hidden');
                document.getElementById('import-step-format').classList.remove('hidden');
            ...
            confirmBulkImport() {
                const grouped = this._pendingImportRows;
                if (!grouped || Object.keys(grouped).length === 0) return;

                let kelasBaruCount = 0, siswaBaruCount = 0;
                Object.keys(grouped).forEach(kelasNama => {
                    // Cari kelas yang sudah ada (case-insensitive), atau buat baru
                    let kelasItem = state.kelas.find(k => k.nama.toLowerCase() === kelasNama.toLowerCase());
                    if (!kelasItem) {
                        kelasItem = { id: system.generateId('kls'), nama: kelasNama, waWaliKelas: '' };
                        state.kelas.push(kelasItem);
                        kelasBaruCount++;
                    }
                    grouped[kelasNama].forEach(siswaNama => {
                        // Sama seperti fitur paste manual: lewati kalau nama+kelas ini sudah ada,
                        // supaya aman diimport ulang tanpa bikin data dobel.
                        const sudahAda = state.siswa.some(s => s.nama.toLowerCase() === siswaNama.toLowerCase() && s.kelas === kelasItem.nama);
                        if (!sudahAda) {
                            state.siswa.push({ id: system.generateId('sw'), nama: siswaNama, kelas: kelasItem.nama });
                            siswaBaruCount++;
                        }
                    });
                });
                ...
                system.saveData('all');
                system.showToast(`Import selesai: ${kelasBaruCount} kelas baru, ${siswaBaruCount} siswa baru ditambahkan.`, 'success');
        ...
        // --- MASTER DATA GURU/PELATIH ---
        // Guru/pelatih diinput sekali di sini (halaman guru.html, mirip Master Kelas), lalu
        // dipilih lewat dropdown di form pembimbing ekskul manapun dia membina -- supaya guru
        // yang membina >1 ekskul tidak perlu diketik ulang tiap kali. Field "pembimbing" pada
        // tiap ekskul (state.ekskul[].pembimbing, array) sekarang berisi ARRAY BERISI ID guru
        // (merujuk ke state.guru[].id), bukan lagi teks bebas. Data lama (sebelum fitur ini
        // ada) yang isinya masih teks bebas tetap didukung secara "read-only" -- lihat
        // getTeacherLabel() di bawah, ditampilkan apa adanya sampai admin memilih ulang guru
        // yang benar dari master data lewat form Kelola Ekskul.
        const teacherManager = {
            pendingPhotoDataUrl: null, // diisi handlePhotoUpload() sebelum form disubmit

            // Cari nama guru dari ID (dipakai untuk menampilkan pembimbing di tabel/kartu/detail).
            // Kalau id tidak ditemukan di master (data lama berupa teks bebas), tampilkan apa
            // adanya supaya admin tetap tahu isi lama itu apa sebelum menggantinya.
            findById(id) {
                return state.guru.find(g => g.id === id) || null;
            },
            getTeacherLabel(id) {
                const g = this.findById(id);
                return g ? g.nama : String(id || '-');
            },

            // Dipakai masterDataManager.addPembimbingField() untuk mengisi dropdown pilih guru.
            buildOptionsHtml(selectedId) {
                const sorted = [...state.guru].sort((a, b) => a.nama.localeCompare(b.nama));
                let html = `<option value="">-- Pilih Guru / Pelatih --</option>`;
                if (sorted.length === 0) {
                    html += `<option value="" disabled>Belum ada data di Master Guru & Pelatih</option>`;
                }
                html += sorted.map(g => `<option value="${g.id}" ${g.id === selectedId ? 'selected' : ''}>${escapeHtml(g.nama)}</option>`).join('');
                // Data lama: id tersimpan tidak cocok dengan guru manapun di master (kemungkinan
                // besar ini teks bebas dari sebelum fitur Master Guru ada) -- tetap tampilkan
                // sebagai opsi terpilih supaya tidak diam-diam hilang, tapi ditandai jelas.
                if (selectedId && !sorted.some(g => g.id === selectedId)) {
                    html += `<option value="${String(selectedId).replace(/"/g, '&quot;')}" selected>(Data lama) ${escapeHtml(selectedId)} -- pilih ulang guru yang benar</option>`;
                }
                return html;
            },

            // --- Upload & kompresi foto (konsisten dengan settingsManager.handleLogoUpload) ---
            handlePhotoUpload(input) {
                ...
                reader.onload = (e) => {
                    const img = new Image();
                    img.onload = () => {
                        const MAX_DIM = 400; // sedikit lebih besar dari logo (256px) supaya foto wajah tetap jelas
                        let { width, height } = img;
                        if (width > height && width > MAX_DIM) { height *= MAX_DIM / width; width = MAX_DIM; }
                        else if (height > MAX_DIM) { width *= MAX_DIM / height; height = MAX_DIM; }
                        ...
                        let dataUrl = canvas.toDataURL('image/jpeg', 0.85);
                        if (dataUrl.length > 90000) dataUrl = canvas.toDataURL('image/jpeg', 0.6);
                        if (dataUrl.length > 90000) dataUrl = canvas.toDataURL('image/jpeg', 0.4);

                        if (dataUrl.length > 90000) {
                            system.showToast('Foto masih terlalu besar, coba pakai foto lain yang lebih sederhana.', 'error');
                            return;
                        }

                        this.pendingPhotoDataUrl = dataUrl;
                        document.getElementById('guru-url').value = ''; // upload & link saling menggantikan
                        this.renderPhotoPreview(dataUrl);
                        system.showToast('Foto siap. Klik "Simpan" untuk menerapkan.', 'success');
                    };
                    img.onerror = () => system.showToast('Gagal membaca gambar.', 'error');
                    img.src = e.target.result;
                };
            ...
            // Saat admin mengetik/tempel link foto, upload yang sebelumnya sudah dipilih (kalau
            // ada) dibatalkan -- supaya jelas cuma 1 sumber foto yang dipakai (upload ATAU link).
            handlePhotoUrlInput() {
                this.pendingPhotoDataUrl = null;
                document.getElementById('guru-photo-file').value = '';
                const url = system.normalizeImageUrl(document.getElementById('guru-url').value.trim());
                this.renderPhotoPreview(url);
            },
            renderPhotoPreview(url) {
                const preview = document.getElementById('guru-photo-preview');
                if (!preview) return;
                if (url) {
                    preview.innerHTML = `<img src="${url}" alt="Preview Foto" class="w-full h-full object-cover" onerror="this.parentElement.innerHTML='<i data-lucide=&quot;user-round&quot; class=&quot;w-8 h-8 text-slate-400&quot;></i>'">`;
                } else {
                    preview.innerHTML = `<i data-lucide="user-round" class="w-8 h-8 text-slate-400"></i>`;
                }
                lucide.createIcons({ root: preview });
            },

            // --- CRUD Admin (halaman guru.html) ---
            renderTable() {
                const grid = document.getElementById('grid-guru');
                if (!grid) return;
                if (state.guru.length === 0) {
                    grid.innerHTML = `<div class="col-span-full p-8 text-center bg-white/40 rounded-2xl border border-dashed border-slate-300 text-slate-500 text-sm"><i data-lucide="user-round-x" class="w-8 h-8 mx-auto text-slate-300 mb-2"></i>Belum ada data guru/pelatih. Klik "Tambah Guru/Pelatih" untuk menambahkan.</div>`;
                    lucide.createIcons({ root: grid });
                    return;
                }
                ...
                grid.innerHTML = sorted.map(g => {
                    const jumlahEkskul = state.ekskul.filter(e => system.toArr(e.pembimbing).includes(g.id)).length;
                    const fotoUrl = g.foto ? system.normalizeImageUrl(g.foto) : '';
                    return `
                    <div class="glass-card p-5 rounded-2xl flex flex-col gap-3">
                        <div class="flex items-center gap-3">
                            <div class="w-14 h-14 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden flex-shrink-0 border border-slate-200 shadow-sm">
                                ${fotoUrl ? `<img src="${fotoUrl}" alt="${escapeHtml(g.nama)}" class="w-full h-full object-cover" loading="lazy" decoding="async" onerror="this.parentElement.innerHTML='<i data-lucide=&quot;user-round&quot; class=&quot;w-6 h-6 text-slate-400&quot;></i>'">` : `<i data-lucide="user-round" class="w-6 h-6 text-slate-400"></i>`}
                            </div>
                            <div class="min-w-0">
                                <h4 class="font-extrabold text-slate-900 text-sm leading-tight truncate">${escapeHtml(g.nama)}</h4>
                                <p class="text-[10px] text-indigo-600 font-semibold mt-0.5">${jumlahEkskul > 0 ? `Membina ${jumlahEkskul} ekskul` : 'Belum membina ekskul'}</p>
                            </div>
                        </div>
                        <p class="text-xs text-slate-600 leading-relaxed line-clamp-2 min-h-[2.2em]">${g.bio ? escapeHtml(g.bio) : '<span class="text-slate-400 italic">Belum ada bio singkat.</span>'}</p>
                        <div class="flex gap-2 pt-2 border-t border-slate-100">
                            <button onclick="teacherManager.openModal('${g.id}')" class="flex-1 py-2 bg-white border border-slate-200 hover:bg-indigo-50 hover:text-indigo-600 text-slate-500 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5"><i data-lucide="pencil" class="w-3.5 h-3.5"></i> Edit</button>
                            <button onclick="teacherManager.deleteTeacher('${g.id}', '${g.nama.replace(/'/g, "\\'")}')" class="flex-1 py-2 bg-white border border-slate-200 hover:bg-rose-50 hover:text-rose-600 text-slate-500 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i> Hapus</button>
                        </div>
                    </div>`;
                }).join('');
                lucide.createIcons({ root: grid });
            ...
            openModal(id = null) {
                this.pendingPhotoDataUrl = null;
                const m = document.getElementById('modal-guru'), c = document.getElementById('modal-guru-card');
                if (!m) return;
                m.classList.remove('pointer-events-none'); void m.offsetWidth; m.classList.add('opacity-100'); c.classList.remove('scale-95');

                document.getElementById('guru-photo-file').value = '';
                if (id) {
                    document.getElementById('modal-guru-title').innerText = 'Edit Guru/Pelatih';
                    const item = state.guru.find(g => g.id === id);
                    if (item) {
                        document.getElementById('guru-id').value = item.id;
                        document.getElementById('guru-nama').value = item.nama;
                        document.getElementById('guru-bio').value = item.bio || '';
                        document.getElementById('guru-url').value = item.foto || '';
                        this.renderPhotoPreview(item.foto ? system.normalizeImageUrl(item.foto) : '');
                    }
                } else {
                    document.getElementById('modal-guru-title').innerText = 'Tambah Guru/Pelatih Baru';
                    document.getElementById('guru-id').value = '';
                    document.getElementById('guru-nama').value = '';
                    document.getElementById('guru-bio').value = '';
                    document.getElementById('guru-url').value = '';
            ...
            saveTeacher(e) {
                ...
                const bio = document.getElementById('guru-bio').value.trim();
                // Foto: prioritaskan upload dari HP (dataURL sudah dikompres) kalau ada,
                // kalau tidak pakai link yang ditempel (sudah dinormalisasi normalizeImageUrl,
                // termasuk otomatis mengonversi link share Google Drive).
                const foto = this.pendingPhotoDataUrl || system.normalizeImageUrl(document.getElementById('guru-url').value.trim());

                if (!nama) {
                    return system.showToast('Nama guru/pelatih wajib diisi.', 'error');
                }

                if (id) {
                    const idx = state.guru.findIndex(g => g.id === id);
                    if (idx !== -1) state.guru[idx] = { id, nama, bio, foto };
                    system.showToast('Data guru/pelatih diperbarui.');
                } else {
                    state.guru.push({ id: system.generateId('gr'), nama, bio, foto });
                    system.showToast('Guru/pelatih berhasil ditambahkan.');
                }

                this.pendingPhotoDataUrl = null;
                system.saveData('guru');
                this.closeModal();
                this.renderTable();
            },

            deleteTeacher(id, nama) {
                const dipakaiDi = state.ekskul.filter(e => system.toArr(e.pembimbing).includes(id));
                const pesanTambahan = dipakaiDi.length
                    ? ` Guru ini masih terdaftar sebagai pembimbing di ${dipakaiDi.length} ekskul (${dipakaiDi.map(e => e.nama).join(', ')}) -- data pembimbing di ekskul tersebut TIDAK akan otomatis terhapus, tapi namanya tidak akan tampil lagi sampai diganti manual lewat menu Kelola Ekskul.`
                    : '';
                uiManager.confirm(`Hapus data guru/pelatih "${nama}"?${pesanTambahan}`, () => {
                    state.guru = state.guru.filter(g => g.id !== id);
                    system.saveData('guru');
                    this.renderTable();
                    system.showToast('Data guru/pelatih berhasil dihapus.', 'success');
                });
            },

            // --- Halaman publik "Guru & Pelatih" (guru-publik.html) ---
            // Daftar SEMUA guru sekali tampil (tidak dobel walau membina beberapa ekskul),
            // cocok dipamerkan ke wali murid baru/PPDB.
            renderPublicList() {
                const grid = document.getElementById('grid-guru-publik');
                if (!grid) return;

                if (state.guru.length === 0) {
                    grid.innerHTML = `<div class="col-span-full p-10 text-center bg-white/60 rounded-3xl border border-dashed border-slate-300 text-slate-500 text-sm"><i data-lucide="users" class="w-10 h-10 mx-auto text-slate-300 mb-3"></i>Data guru & pelatih belum tersedia saat ini.</div>`;
                    lucide.createIcons({ root: grid });
                    return;
                }
                ...
                grid.innerHTML = sorted.map(g => {
                    const fotoUrl = g.foto ? system.normalizeImageUrl(g.foto) : '';
                    const ekskulDibina = state.ekskul.filter(e => system.toArr(e.pembimbing).includes(g.id)).map(e => e.nama);
                    return `
                    <div class="glass-card p-6 rounded-3xl flex flex-col items-center text-center gap-3">
                        <div class="w-24 h-24 rounded-full bg-slate-200 flex items-center justify-center overflow-hidden border-4 border-white shadow-md">
                            ${fotoUrl ? `<img src="${fotoUrl}" alt="${escapeHtml(g.nama)}" class="w-full h-full object-cover" loading="lazy" decoding="async" onerror="this.parentElement.innerHTML='<i data-lucide=&quot;user-round&quot; class=&quot;w-10 h-10 text-slate-400&quot;></i>'">` : `<i data-lucide="user-round" class="w-10 h-10 text-slate-400"></i>`}
                        </div>
                        <div>
                            <h4 class="font-extrabold text-slate-900 text-base leading-tight">${escapeHtml(g.nama)}</h4>
                            ${ekskulDibina.length ? `<p class="text-[11px] text-indigo-600 font-semibold mt-1">${ekskulDibina.map(escapeHtml).join(' &bull; ')}</p>` : ''}
                        </div>
                        ${g.bio ? `<p class="text-xs text-slate-600 leading-relaxed text-justify">${escapeHtml(g.bio)}</p>` : ''}
                    </div>`;
                }).join('');
                lucide.createIcons({ root: grid });
            }
        ...
        const masterDataManager = {
            renderTable() {
                const tbody = document.getElementById('table-body-ekskul');
                // Elemen ini cuma ada di kelola.html. Saat ini tidak ada jalur di UI yang
                // memanggil renderTable() dari halaman lain, tapi null-check ini tetap
                // dipasang sebagai jaring pengaman kalau suatu saat ada pemanggilan baru
                // yang lolos dari halaman lain (sama seperti pola null-check di uiManager).
                if (!tbody) return;
                tbody.innerHTML = '';
                ...
                state.ekskul.forEach(item => {
                    ...
                    tbody.innerHTML += `
                        <tr class="hover:bg-indigo-50/40 border-b border-slate-100 last:border-0">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-xs font-semibold text-slate-700 space-y-0.5">${pembimbingArr.length ? pembimbingArr.map(p => `<span class="block">${escapeHtml(teacherManager.getTeacherLabel(p))}</span>`).join('') : '-'}</p>
            ...
            // Sejak fitur Master Data Guru/Pelatih ada, field ini dropdown pilih guru dari
            // master (bukan teks bebas lagi) -- supaya guru yang membina >1 ekskul cukup
            // diinput sekali lalu tinggal dipilih di ekskul manapun dia mengajar. "value" di
            // sini berisi ID guru (state.guru[].id); lihat teacherManager.buildOptionsHtml()
            // untuk cara data lama (teks bebas) tetap ditampilkan supaya tidak hilang diam-diam.
            addPembimbingField(value = '') {
                ...
                row.innerHTML = `
                    <select class="ekskul-pembimbing-input flex-1 px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20">
                        ${teacherManager.buildOptionsHtml(value)}
                    </select>
            ...
            saveEkskul(e) {
                ...
                if(pembimbing.length === 0) {
                    return system.showToast('Pilih minimal 1 guru/pelatih (dari Master Guru & Pelatih).', 'error');
                }
                ...
                container.innerHTML = filtered.map(u => `
                    <div class="glass-card p-5 rounded-2xl flex flex-col gap-3">
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <h4 class="font-extrabold text-slate-900 text-base leading-tight">${escapeHtml(u.usulanNama)}</h4>
                                <p class="text-[11px] text-slate-500 mt-1">Diusulkan oleh <span class="font-semibold text-slate-700">${escapeHtml(u.nama)}</span> &bull; Kelas ${escapeHtml(u.kelas)}</p>
                            </div>
                        </div>
                        ${u.usulanAlasan ? `<p class="text-xs text-slate-600 bg-slate-50 border border-slate-100 rounded-xl p-3 leading-relaxed">"${escapeHtml(u.usulanAlasan)}"</p>` : ''}
        ...
        const saranManager = {
            ...
            async submitPublic(e) {
                ...
                try {
                    const saranBaru = state.saran.concat([{
                    ...
                    // Tunggu konfirmasi server dulu -- supaya tidak menutup modal & bilang
                    // "terkirim" padahal gagal tersimpan karena koneksi jelek.
                    const result = await api.pushAwait('saran', saranBaru);
                    if (!result.success) {
                        system.showToast('Gagal mengirim: ' + result.error + ' Isian Anda tidak hilang -- silakan coba lagi.', 'error');
                        return;
                    }

                    state.saran = saranBaru;
                    localStorage.setItem(DB_KEY_SARAN, JSON.stringify(state.saran));
                    this.updateBadge();
                    form.reset();
            ...
            renderTable() {
                ...
                container.innerHTML = filtered.map(s => `
                    <div class="glass-card p-5 rounded-2xl flex flex-col gap-3">
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <h4 class="font-extrabold text-slate-900 text-base leading-tight">${escapeHtml(s.nama)}</h4>
                                <p class="text-[11px] text-slate-500 mt-1">No. WA: <span class="font-semibold text-slate-700">${escapeHtml(s.noWa)}</span>${s.jenisEkskul ? ` &bull; Terkait: <span class="font-semibold text-slate-700">${escapeHtml(s.jenisEkskul)}</span>` : ''}</p>
                            </div>
                        </div>
                        ${s.kritik ? `<div><p class="text-[10px] font-bold text-rose-500 uppercase tracking-wide mb-1">Kritik</p><p class="text-xs text-slate-600 bg-slate-50 border border-slate-100 rounded-xl p-3 leading-relaxed">"${escapeHtml(s.kritik)}"</p></div>` : ''}
                        ${s.saran ? `<div><p class="text-[10px] font-bold text-emerald-600 uppercase tracking-wide mb-1">Saran</p><p class="text-xs text-slate-600 bg-slate-50 border border-slate-100 rounded-xl p-3 leading-relaxed">"${escapeHtml(s.saran)}"</p></div>` : ''}
                        ${(!s.kritik && !s.saran && s.pesan) ? `<p class="text-xs text-slate-600 bg-slate-50 border border-slate-100 rounded-xl p-3 leading-relaxed">"${escapeHtml(s.pesan)}"</p>` : ''}
        ...
        const galleryManager = {
            ...
            // --- Admin Functions ---
            renderAdminTable() {
                const tbody = document.getElementById('table-body-galeri');
                // Elemen ini cuma ada di galeri.html -- null-check sebagai jaring pengaman,
                // sama seperti masterDataManager.renderTable() di atas.
                if (!tbody) return;
                tbody.innerHTML = '';
                ...
                [...filtered].sort((a,b) => a.nama.localeCompare(b.nama)).forEach((item, idx) => {
                    tbody.innerHTML += `
                        <tr class="hover:bg-indigo-50/40 border-b border-slate-100 last:border-0">
                            ...
                            <td class="px-6 py-3 font-semibold text-slate-800">${escapeHtml(item.nama)}</td>
                            <td class="px-6 py-3 text-xs"><span class="bg-slate-100 text-slate-600 border border-slate-200 px-2 py-1 rounded">${escapeHtml(item.kelas)}</span></td>
                ...
                [...filtered].reverse().forEach((row, index) => {
                    ...
                    tbody.innerHTML += `
                        <tr class="${(index+1)%2===0?'bg-slate-50/50':'bg-transparent'} hover:bg-indigo-50/60 font-mono text-[13px] border-b border-slate-200/50">
                            ...
                            <td class="px-4 py-3 border-r font-bold text-slate-800 font-sans">${escapeHtml(row.nama)}</td>
                            <td class="px-4 py-3 border-r font-sans text-xs"><span class="bg-slate-100 px-2 py-0.5 rounded border border-slate-200">${escapeHtml(row.kelas)}</span></td>
                            <td class="px-4 py-3 border-r font-sans text-xs text-center">${row.jenisKelamin === 'P' ? 'P' : (row.jenisKelamin === 'L' ? 'L' : '-')}</td>
                            <td class="px-4 py-3 border-r font-sans">${escapeHtml(row.namaOrtu) || '-'}</td>
                            <td class="px-4 py-3 border-r font-sans">${escapeHtml(row.waOrtu) || '-'}</td>
                            <td class="px-4 py-3 border-r font-semibold text-emerald-700 font-sans max-w-[250px] truncate whitespace-normal leading-relaxed">${escapeHtml(pilihanStr)}</td>
                            <td class="px-4 py-3 border-r text-slate-500 font-sans truncate max-w-[200px]" title="${escapeHtml(row.alasan)}">${escapeHtml(row.alasan)}</td>
        ...
        const cetakManager = {
            init() {
                ...
                const tahunAjaranEl = document.getElementById('cetak-tahun-ajaran');
                if (tahunAjaranEl) {
                    const ta = state.settings && state.settings.tahunAjaran;
                    tahunAjaranEl.textContent = ta ? `Tahun Ajaran ${ta}` : '';
                }
            ...
            buildBarisPeserta(r, no) {
                return `
                    <tr>
                        <td class="cetak-td text-center">${no}</td>
                        <td class="cetak-td">${escapeHtml(r.nama)}</td>
                        <td class="cetak-td text-center">${escapeHtml(r.kelas) || '-'}</td>
            ...
            // namaKelas kosong ("Semua Kelas") & peserta berasal dari >1 kelas -> beri judul pemisah
            // per kelas (mis. "Kelas 1A") supaya daftar panjang tetap mudah dipindai cepat.
            buildTabel(peserta, namaKelasFilter) {
                if (peserta.length === 0) {
                    return `<tr><td colspan="5" class="text-center py-6 text-slate-400 italic">Belum ada peserta yang mendaftar.</td></tr>`;
                }

                const distinctKelas = new Set(peserta.map(r => r.kelas || 'Tanpa Kelas'));
                const perluDipisah = !namaKelasFilter && distinctKelas.size > 1;

                if (!perluDipisah) {
                    return peserta.map((r, idx) => this.buildBarisPeserta(r, idx + 1)).join('');
                }
                ...
                peserta.forEach(r => {
                    const labelKelas = r.kelas || 'Tanpa Kelas';
                    if (labelKelas !== kelasBerjalan) {
                        kelasBerjalan = labelKelas;
                        const jumlahDiKelasIni = peserta.filter(p => (p.kelas || 'Tanpa Kelas') === labelKelas).length;
                        html += `<tr class="cetak-kelas-sep-row"><td colspan="5" class="cetak-kelas-sep">Kelas ${escapeHtml(labelKelas)} <span>&middot; ${jumlahDiKelasIni} siswa</span></td></tr>`;
                    }
                    no++;
                    html += this.buildBarisPeserta(r, no);
                });
                return html;
            },
                ...
                return `
                    <div class="cetak-blok">
                        ...
                        <table class="cetak-tabel">
                            ...
                            <tbody>${this.buildTabel(peserta, namaKelas)}</tbody>
        ...
        const dashboardManager = {
            init() {
                ...
                // Header khusus cetak (hanya kelihatan saat print, lihat .dash-print-header di style.css)
                const printSekolahEl = document.getElementById('dash-print-nama-sekolah');
                if (printSekolahEl) printSekolahEl.textContent = (state.settings && state.settings.schoolName) || DEFAULT_SETTINGS.schoolName;
                const printTaEl = document.getElementById('dash-print-tahun-ajaran');
                if (printTaEl) printTaEl.textContent = state.settings.tahunAjaran ? `Tahun Ajaran ${state.settings.tahunAjaran}` : '';
                const printTglEl = document.getElementById('dash-print-tanggal');
                if (printTglEl) printTglEl.textContent = new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
                ...
                container.innerHTML = recent.map(sv => {
                    ...
                    return `
                        <div class="flex items-start gap-3 py-2.5 border-b border-slate-100 last:border-0 px-2">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-bold text-slate-800 truncate">${escapeHtml(sv.nama)} <span class="font-medium text-slate-400">&middot; ${escapeHtml(sv.kelas)}</span></p>
            ...
            // Cetak ringkasan dashboard (KPI, 3 chart, partisipasi per kelas, ekskul sepi peminat)
            // sebagai laporan 1-2 halaman untuk kepala sekolah. Bagian yang murni interaktif/
            // operasional (tombol AI, log aktivitas terbaru) sengaja disembunyikan saat print
            // lewat class "dash-no-print-inline" -- lihat style.css.
            cetakLaporan() {
                // Buka otomatis semua daftar "belum isi" per kelas supaya kepala sekolah bisa
                // langsung lihat nama-namanya di kertas (di layar tetap tersembunyi sampai diklik;
                // tombol togglenya sendiri ikut disembunyikan khusus saat print, lihat style.css).
                document.querySelectorAll('[id^="kelas-belum-"]').forEach(el => el.classList.remove('d-none'));
                window.print();
            }
        };
